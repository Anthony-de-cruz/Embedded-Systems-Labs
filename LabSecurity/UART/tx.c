...
int main(void){
	...
	char message[] = "Hello World from TX"; //message
	...
	for(;;)
	{
		HAL_UART_Transmit(&s_UARTHandle,(uint8_t*)message, strlen(message), HAL_MAX_DELAY);
		//send end char
		HAL_UART_Transmit(&s_UARTHandle,"\0", sizeof("\0"), HAL_MAX_DELAY);
		delayms(100);
	}
